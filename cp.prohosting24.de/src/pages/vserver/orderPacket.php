...
echo minifyhtml(getnormalbody($config, $lang->getString("vserverorderheaderplans"), $user, $lang));
...
$packetInfo = requestBackend($config, [], "getpackets", $user->getLang())["response"];
$packetData = [];

foreach ($packetInfo as $key => $value) {
    $packetData[$value["id"]] = $value;
}
if(isset($packetId)){
    if(!isset($packetData[$packetId])){
        $this->sendclient("404", $router, $config, $content, $user, $lang);
        die();
    }
} else {
    $packetDataTmp = [];
    $count = 0;
    foreach ($packetData as $key =>$data) {
        if($data["normal"] == 1){
            $packetDataTmp[$key] = $data;
            $count++;
        }
    }
    $packetData = $packetDataTmp;
    $packetInfoTmp = [];
    $count = 0;
    foreach ($packetInfo as $key => $value) {
        if($value["normal"] == 1){
            $packetInfoTmp[$count] = $value;
            $count++;
        }
    }
    $packetInfo = $packetInfoTmp;
}
                                                ...
                                                <div class="bg-gray-100 d-flex align-items-center p-5 rounded gutter-b">
                                                    ...
                                                    <div class="ml-1">
                                                        <h3
                                                            ...
                                                            <text id="cores_step_1"></text> <?php echo $lang->getString("cpucores") ?></h3>
                                                    </div>
                                                    <!--end::Description-->
                                                </div>
                                                <div class="bg-gray-100 d-flex align-items-center p-5 rounded gutter-b">
...
<script>
    var price = 0;
    var duration = 30;
    var packetId = <?php if(isset($packetId)){echo $packetId;} else{ echo 0;} ?>;
    ...
    function updateData() {
        ...
        if(packetInfo[packetId].normal != 1){
            $('#button_order_open_discount').hide();
        } else {
            $('#button_order_open_discount').show();
        }
        ...
        $('#ipv4_step_1').html(1);
        $('#memory_step_1').html(packetData.memory / 1024);
        $('#disk_step_1').html(packetData.disk);
        $('#cores_step_1').html(packetData.cores);

        $('#ipv4_step_4').html(1);
        $('#memory_step_4').html(packetData.memory / 1024);
        $('#disk_step_4').html(packetData.disk);
        $('#cores_step_4').html(packetData.cores);
    ...
    jQuery(document).ready(function () {
        ...
        packetSlider.noUiSlider.on('update', function (values, handle) {
            sliderPacketInput.value = packetDataA[values]["title"];
    ...
    function checkDiscount() {
        if(packetInfo[packetId].normal != 1){
            toastr["error"]('<?php echo $lang->getString("vservernodiscount") ?>');
            return;
        }
        ...
        requestIntern({ sessionid: Cookies.get('ph24_sessionid'), code: discountcode, productid: 100 }, "checkdiscountcode", function (respond) {
            if (respond.fail) {
    ...
    function order() {
        ...
        requestIntern({ sessionid: Cookies.get('ph24_sessionid'), packageid:packetId,imageid:currentOs,discountcode:discountcode, days:currentDuration }, "orderpackage", function (respond) {
            if (respond.fail) {
                toastr["error"](respond.error);
