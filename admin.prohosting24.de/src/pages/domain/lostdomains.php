...
echo minifyhtml(getheader($config, "LostDomains - ProHosting24"));
...
echo minifyhtml(getnormalbody($config, "Domain Übersicht", $user));
    ...
    <div class="col-xl-12">
        <div class="kt-portlet kt-portlet--mobile">
            ...
            <div class="kt-portlet__body">
                ' . getloadinghtml() . '
        		<table class="table table-striped-   table-hover" id="domains_tabelle" style="display:none">
        			<thead>
        				<tr>
        					<th>#</th>
                            <th>Proxmoxid</th>
...
function getvms(){
    $.ajax({
        ...
        beforeSend: function(request) {
            request.setRequestHeader('Function', 'getlostdomains');
        },
        ...
        success: function(respond){
            if(respond.fail){
                ...
                i = 0;
                $('#domains_tabelle').DataTable().clear().draw();
                respond.response.forEach(element => {
                    $('#domains_tabelle').DataTable().row.add( [
                        i,
                        element,
                    ] ).draw( false );
                    i++;
                ...
                document.getElementById('domains_tabelle').style.display = '';
            }
        }
    });
}
$('#domains_tabelle').DataTable({
    responsive: true,
    pageLength: 50,
